    ...
    match objects {
        ...
        areas_volumes::GeometricalVolumes::Pyramid => {
            triangular_pyramid_volume(a as f64, b) * times as f64
                <= cube_volume(rectangle_area(x, y)) as f64
        }
        areas_volumes::GeometricalVolumes::Parallelepiped => {
            parallelepiped_volume(a, b, c) * times <= cube_volume(rectangle_area(x, y))
        }
        areas_volumes::GeometricalVolumes::Cone => {
            cone_volume(a, b) * times as f64 <= cube_volume(rectangle_area(x, y)) as f64
        }
    }
}
